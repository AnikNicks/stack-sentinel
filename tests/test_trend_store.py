...
def test_duplicate_append_does_not_create_a_second_record(isolated_trend_store):
    ...
    entry for the same (company_id, quarter) must result in exactly one record on file."""
...
def test_duplicate_key_different_content_still_no_op(isolated_trend_store):
    """Idempotency is keyed by (company_id, quarter) only — even a differently-worded
    ...
    history = isolated_trend_store.get_trend_history("acme")
    assert len(history) == 1
    assert history[0]["classification"] == "on_thesis", "original entry must be preserved, not overwritten"
...
def test_get_trend_history_limit_returns_most_recent(isolated_trend_store):
    ...
    assert [e["quarter"] for e in full] == ["2025-Q1", "2025-Q2", "2025-Q3", "2025-Q4"]

    bounded = isolated_trend_store.get_trend_history("acme", limit=2)
    assert [e["quarter"] for e in bounded] == ["2025-Q3", "2025-Q4"], "limit must return the MOST RECENT entries"
