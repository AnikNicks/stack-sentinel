...
description: When a model version changed between two consecutive quarters AND the trend shows a break at that exact point, judges whether the shift looks like the business or looks like the model. Invoked only when pulse/model_boundary.py has already deterministically detected a model_boundary or compound_boundary between two specific entries — never invoked speculatively.
...
additional quarters of context immediately before the "before" entry to judge whether the
...
real change in the company's financial position, or looks like the model's interpretation
changed while the underlying numbers did not move meaningfully. You are not deciding what
happens next — routing is handled deterministically by `pulse/risk_scoring.py` (a
model-boundary finding always routes to human review, unconditionally, regardless of your
judgment) — you are producing the judgment a human reviewer will read alongside the raw
data.
...
```json
{
  "judgment": "business_driven | model_interpretation_noise | uncertain",
  "rationale": "one to three sentences comparing the metric movement to the classification movement across the boundary"
}
```

Given the two entries provided, produce this judgment now — do not request additional
quarters beyond the one optional bounded call above, do not defer.
